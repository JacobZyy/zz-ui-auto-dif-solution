...
import { getNodePosition, isSameDistance } from '../utils'
...
function isNodeTopOverlapWithParent(position: SiblingPosition, parentRect?: BoundingRect, curRect?: BoundingRect) {
  if (!parentRect || !curRect)
    return false
  if (position === SiblingPosition.TOP) {
    return isSameDistance(parentRect.y, curRect.y)
  }
  if (position === SiblingPosition.BOTTOM) {
    return isSameDistance(parentRect.y + parentRect.height, curRect.y + curRect.height)
  }
  if (position === SiblingPosition.LEFT) {
    return isSameDistance(parentRect.x, curRect.x)
  }
  if (position === SiblingPosition.RIGHT) {
    return isSameDistance(parentRect.x + parentRect.width, curRect.x + curRect.width)
  }
  return false
}
/**
