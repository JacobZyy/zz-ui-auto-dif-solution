...
import { getDomMarginInfo, getDomNodeByUniqueId } from '../utils'
...
function judgeMarginCollapsing({ currentNodeInfo, position }: { currentNodeInfo: NodeInfo, position: MarginInfoDirection }) {
  ...
  const borderWidth = borderInfo?.[borderKey]?.width
  const borderColor = borderInfo?.[borderKey]?.color
...
function getMarginInfoForMerge(currentNodeInfo: NodeInfo, flatNodeMap: Map<UniqueId, NodeInfo>): Record<MarginInfoDirection, number> {
  ...
  const topChildNodeMarginInfo = children
    ...
    .map((childId) => {
      const childNodeInfo = flatNodeMap.get(childId)
      const element = getDomNodeByUniqueId(childId)
 ...
 * @returns 处理后的扁平化节点信息
 */
export function processMarginCollapsing(flatNodeMap: Map<UniqueId, NodeInfo>) {
  const result = produce(flatNodeMap, (draftFlatNodeMap) => {
    draftFlatNodeMap.forEach((currentNodeInfo, nodeId) => {
      ...
      const marginInfoForMerge = getMarginInfoForMerge(currentNodeInfo, draftFlatNodeMap)
      ...
      draftFlatNodeMap.set(nodeId, currentNodeInfo)
    })
  })
  return result
}
