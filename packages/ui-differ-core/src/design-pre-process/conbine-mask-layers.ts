...
import { MASK_REPLACE_SUFFIX, nodeCanBeMaskSet, nodeNoChildSet } from '../types'
...
export function combineMaskLayers<T extends SceneNode>(currentNode: T): T {
  ...
  const firstMaskRelpaceNodeInfo = newChildGroupList[0]
  if (!firstMaskRelpaceNodeInfo) {
    return currentNode
  }

  const isSameSize = currentNode.width === firstMaskRelpaceNodeInfo.width && currentNode.height === firstMaskRelpaceNodeInfo.height
