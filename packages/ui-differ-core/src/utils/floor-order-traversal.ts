...
import { UnVisibleNodeTag } from '../dom-handlers'
import { ignoreChildrenClsSet, nodeNoChildSet } from '../types'
...
export function* floorOrderTraversalWithDom(rootDom: Element): Generator<Element, void, unknown> {
  ...
  while (queue.length > 0) {
    ...
    if (!currentDom)
      continue
    const isUnVisibleNode = currentDom.getAttribute('un-visible-node') === UnVisibleNodeTag.ALL
    if (isUnVisibleNode) {
      continue
    }
    ...
    const classNameList = Array.from(currentDom.classList.values())

    const shouldIgnoreChildren = classNameList.some(className => ignoreChildrenClsSet.has(className))
...
export function* floorOrderTraversalWithNode(rootNode: SceneNode): Generator<SceneNode, void, unknown> {
  ...
  while (queue.length > 0) {
    ...
    // mask替代节点忽略子节点
    if (currentNode.name.includes('mask-replace')) {
      continue
    }
