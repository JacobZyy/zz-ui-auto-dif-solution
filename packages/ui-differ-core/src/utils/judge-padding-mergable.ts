...
export function judgePaddingMergable({ currentNodeInfo, position, flatNodeMap }: JudgeMergableConfig): boolean {
  ...
  const targetBorderInfo = currentNodeInfo.borderInfo[borderKey]
  ...
  const hasTargetBorderInfo = !!targetBorderInfo.width && targetBorderInfo.color !== 'transparent' && targetBorderInfo.color !== currentBgColor
