...
export async function handleChangeWindowSize(configs: ChromeListenerMsgProcessorParams): Promise<ChromeListenerMsgResponse> {
  const { sender, message } = configs

  console.log('🚀 ~ handleChangeWindowSize ~ message:', message)
  const targetHeight = message?.data?.targetHeight || DEFAULT_PAGE_HEIGHT
  const targetWidth = message?.data?.targetWidth || DEFAULT_PAGE_WIDTH
