...
| 场景 | 技术栈 |
|------|--------|
| PC端 | React 18 + Umi4 + Ant Design 5 + ProComponents 2 |
...
- [React 开发规范](.claude/rules/frameworks-react.md) — React 组件、Hooks、样式、性能（`*.jsx` / `*.tsx`）
- [状态与数据层规范](.claude/rules/state-data-layer.md) — Zustand、ahooks useRequest、Pro `request`、服务层
- [交互规范](.claude/rules/interaction-standards.md) — 表格、表单、按钮、反馈（`*.jsx` / `*.tsx`）
- [Ant Design 组件规范](.claude/rules/component-library-antd.md) — Pro、antd、zant-ui（强业务）
- [测试规范](.claude/rules/testing.md) — Vitest/Jest、RTL、Hook 测试、Mock 策略（`*.test.tsx`）
...
通用 UI：**ProComponents** > **Ant Design** > 业务自定义。仅 **强业务关联**（上传、省市区、导出/下载等，见 [组件规范](.claude/rules/component-library-antd.md)）在已依赖时优先 **`@zz-common/zant-ui`**，不要用裸 antd 硬接这类能力。
...
| 能力 | 包 / 约定 | 要求 |
|------|-----------|------|
| HTTP | `@zz/fetch` 或本仓库 `request` 封装 | 走统一层，勿再叠一套平行客户端（存量 `umi-request` 未迁完可延续） |
| 登录 / 权限 | `@zz-common/zz-permission`，`getInitialState`，`@umijs/plugin-access` | 沿用本仓库既有接入 |
| zant-ui | `@zz-common/zant-ui` | 仅强业务组件优先用，见 [component-library-antd.md](.claude/rules/component-library-antd.md) |
| 错误上报 | `@zz-common/sentry` | 已接入则与存量用法一致 |
| 工具 | `@zz-common/zz-utils` | 编写前先查是否已有 |
| 接口生成 | ztt / `@zz-common/zapi-to-typescript` | 新接口优先生成到 `api-ts-export` 或仓库约定目录 |
| 工程 | `@zz-common/zz-cli`、`@zz-common/dev-standard` | 以本仓库脚本为准 |

例外在 PR 说明原因与回收方式。

## 双轨升级策略（兼容存量）

- 新代码优先：ahooks `useRequest` + ProComponents `request` + Zustand（与 Umi4 一致，不引入并行请求库）
- 存量代码兼容：useModel/dva + ahooks `useRequest` + umi-request
- 不强制一次性迁移，按功能迭代逐步替换
- 修改存量模块时，优先清理明显反模式（手写 loading、重复请求、类型漂移）

## 老项目降级适配规则

- 若依赖版本、构建链路或框架约束导致约定方案不可落地，允许降级使用存量方案
- 降级优先级：先保证业务正确与可维护，再逐步推进技术升级
- 新增需求若落在老模块中，可沿用该模块主技术栈，避免同文件混搭两套实现
- 一旦基础依赖可升级，后续迭代应优先收敛到 **useRequest + Pro `request` + Zustand** 这一套
...
- 有明确歧义或影响较大时，主动向用户说明判断依据并确认

## 规则执行与例外流程

- 规则默认按“快速决策 -> MUST -> SHOULD -> 例外处理”顺序执行
- 若触发例外，需在 PR 说明：冲突点、降级原因、回收计划
- 同类例外连续出现两次及以上，应回写到规则文件形成显式例外条款

## Umi 项目配置说明

- 遵循 Umi 约定式路由，页面文件放在 `src/pages/`
- 全局状态新代码优先 Zustand，存量可继续使用 `useModel`
- 权限控制使用 `@umijs/plugin-access`
- 接口代理配置在 `config/proxy.ts`
<!-- FE_SPEC_END -->
